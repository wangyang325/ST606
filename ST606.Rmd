...
# 7244
dat$m_date = as.character(dat$m_date)
dat[which(dat$ID=="1251"),]
#dat[which(dat$ID=="7244" & dat$m_date == '2014-10-01'), 'age'] = dat[which(dat$ID=="7244" & dat$m_date == '2014-10-01'), 'age'] + 1
#dat[which(dat$ID=="7244" & dat$m_date == '2014-10-01'), 'age_bin'] = dat[which(dat$ID=="7244" & dat$m_date == '2014-10-01'), 'age_bin'] + 1
#2258
#delRowNo <- which(dat$ID=="2258" & dat$weight == 46)
#dat <- dat[-delRowNo , ] 

## 1251  1309  1471  1629  2258  2727  2769 2770
...
fitPlm <- plm(last_rs~ gender + z_score + new_age_bin, data=dat_Last, model = 'pooling', effect = "twoways", index=c("ID", "new_age_bin"))
summary(fitPlm)

beta <- coef(fitPlm)
X <- model.matrix(fitPlm)
crossprod(t(X), beta)
predict(fitPlm, dat_Last[which(dat_Last$ID=="1"),])
```
```{r}
