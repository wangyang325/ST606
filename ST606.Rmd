...
ggplot(plot_D(fitHB), aes(.x, .y, colour=.id)) + labs(x='age', y='height') + geom_line(show.legend=FALSE) + ggtitle("The growth curve for each boy")

ggplot(plot_a(mba), aes(.x, .y, colour=.id)) + labs(x='age', y='height') + geom_line(show.legend=FALSE) + ggtitle("(a) Random Effect by α(a)")
#summary(mba)

ggplot(plot_a(mbab), aes(.x, .y, colour=.id)) + labs(x='age', y='height') + geom_line(show.legend=FALSE) + ggtitle("(b) Random Effect by α(a) + β(b)")
#summary(mbab)

ggplot(plot_a(fitHB), aes(.x, .y, colour=.id)) + labs(x='age', y='height') + geom_line(show.legend=FALSE) + ggtitle("(c) Random Effect by α(a) + β(b) + γ(c)")

ggplot(plot_d(fitHB), aes(.x, .y)) + labs(x='age', y='height') + geom_line(show.legend=FALSE) + ggtitle("(d) Growth Curve with Random Effect by α(a) + β(b) + γ(c)")
#summary(fitHB)
...
```{r}
mga <- sitar(x=age, y=height, id=ID, data=data_f_girl, df=bestDf[1], random = 'a', control = nlmeControl(pnlsTol = 12))
mgab <- sitar(x=age, y=height, id=ID, data=data_f_girl, df=bestDf[1], random = 'a+b')

ggplot(plot_D(fitHG), aes(.x, .y, colour=.id)) + labs(x='age', y='height') + geom_line(show.legend=FALSE) + ggtitle("The growth curve for each girl")

ggplot(plot_a(mga), aes(.x, .y, colour=.id)) + labs(x='age', y='height') + geom_line(show.legend=FALSE) + ggtitle("(a) Random Effect by α(a)")
#summary(mga)

ggplot(plot_a(mgab), aes(.x, .y, colour=.id)) + labs(x='age', y='height') + geom_line(show.legend=FALSE) + ggtitle("(b) Random Effect by α(a) + β(b)")
#summary(mgab)

ggplot(plot_a(fitHG), aes(.x, .y, colour=.id)) + labs(x='age', y='height') + geom_line(show.legend=FALSE) + ggtitle("(c) Random Effect by α(a) + β(b) + γ(c)")

ggplot(plot_d(fitHG), aes(.x, .y)) + labs(x='age', y='height') + geom_line(show.legend=FALSE) + ggtitle("(d) Growth Curve with Random Effect by α(a) + β(b) + γ(c)")
#summary(fitHG)
...
(a) fit the model only with a random effect α(a). (does not convergence)
...
```{r}
plot(fitHB, col='blue', y2par=list(col='blue'))
lines(fitHG,col='red', y2par=list(col='red'))
title("The Growth Curve (Boy:blue, Girl:red)")
```

1. The growth rate:

   6-10 year-old: Boys and Girls almost have the same rate.
   
   10~13.5 year-old: Girls are a little bit higer than boys.
   
   13.6~18 year-old: Boys are significant high than Girls.
   
2. The growth velocity:

   a) 6~9 year-old: Boys and Girls almost are near to each other.
   b) 9~10.5 year-old: Boys and girls have different trend, Boys delined, Girls increased 
   c) 10.5~17 year-old: Girls keep declining fast, Boys reach the top at 13.5, then declined fast
   d) 17~18 year-old: Grils and Boys are near to each other.